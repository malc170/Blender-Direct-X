...
class DirectXExporter:
    ...
    def __init__(self, Config, context):
        ...
        if self.Config.CoordinateSystem == 'LEFT_HANDED':
            self.SystemMatrix = self.SystemMatrix @ Matrix.Scale(-1, 4, Vector((0, 0, 1)))
    ...
    # "Public" Interface
    def Export(self):
        self.Log("Exporting to {}".format(self.File.FilePath), MessageVerbose=False)
    ...
    def __GatherAnimationGenerators(self):
        ...
        # Otherwise,
            ...
            for Object in self.ExportList:
                ...
                if Object.BlenderObject.type == 'ARMATURE':
                    Generators.append(ArmatureAnimationGenerator(self.Config, Util.SafeName(Object.BlenderObject.animation_data.action.name), Object))
                else:
                    Generators.append(GenericAnimationGenerator(self.Config, Util.SafeName(Object.BlenderObject.animation_data.action.name), Object))
            ...
            if self.Config.AttachToFirstArmature:
                ...
                if FirstArmature is not None:
                    ...
                    for Action in FreeActions:
                        ...
                        Generators.append(ArmatureAnimationGenerator(self.Config, Util.SafeName(Action.name), FirstArmature))
...
class MeshExportObject(ExportObject):
    ...
    def Write(self):
        ...
        if self.Config.ExportMeshes:
            ...
            if self.Config.ApplyModifiers:
                ...
                #Mesh = self.BlenderObject.to_mesh(self.Exporter.context.scene, True, 'PREVIEW') # Change by me two lines below added
                ...
                #Mesh = self.BlenderObject.to_mesh(self.Exporter.context.scene, False, 'PREVIEW') # Change by me two lines below added
                depsgraph = self.Exporter.context.evaluated_depsgraph_get()
                Mesh = self.BlenderObject.evaluated_get(depsgraph).to_mesh()
            ...
            # Cleanup
            #bpy.data.meshes.remove(Mesh) changed by malc 
    ...
    class _UnrolledFacesMeshEnumerator(_MeshEnumerator):
        ...
        def __init__(self, Mesh):
            ...
            for Polygon in Mesh.polygons:
                self.PolygonVertexIndices.append(tuple(range(Index, Index + len(Polygon.vertices))))
    ...
    def __WriteMesh(self, Mesh):
        ...
        if (self.Config.ExportUVCoordinates and Mesh.uv_layers) or (self.Config.ExportVertexColors and Mesh.vertex_colors) or (self.Config.ExportSkinWeights):
        ...
        for Index, Vertex in enumerate(MeshEnumerator.vertices):
            Position = Vertex.co
            self.Exporter.File.Write("{:9f};{:9f};{:9f};".format(Position[0], Position[1], Position[2]))
        ...
        for Index, PolygonVertexIndices in enumerate(MeshEnumerator.PolygonVertexIndices):
            ...
            for VertexCountIndex, VertexIndex in enumerate(PolygonVertexIndices):
        ...
        if self.Config.ExportMaterials:
            ...
            if self.Config.ExportActiveImageMaterials:
                self.Exporter.Log("Referencing active images instead of material image textures.")
    ...
    def __WriteMeshNormals(self, Mesh, MeshEnumerator=None):
            ...
            def __init__(self, Mesh):
                ...
                for Polygon in Mesh.polygons:
                    if not Polygon.use_smooth:
                        ...
                        self.PolygonVertexIndices.append(tuple(len(Polygon.vertices) * [Index]))
                        Index += 1
                    else:
                        for VertexIndex in Polygon.vertices:
        ...
        for Index, Vertex in enumerate(MeshEnumerator.vertices):
            ...
            self.Exporter.File.Write("{:9f};{:9f};{:9f};".format(Normal[0], Normal[1], Normal[2]))
    ...
    def __WriteMeshUVCoordinates(self, Mesh):
        ...
        for Polygon in Mesh.polygons:
            ...
            for Vertex in Vertices:
                self.Exporter.File.Write("{:9f};{:9f};".format(Vertex[0], 1.0 - Vertex[1]))
                Index += 1
    ...
    def __WriteMeshMaterials(self, Mesh):
        def WriteMaterial(Exporter, Material):
            def GetMaterialTextureFileName(Material):
                ...
                # ************* TESTING OUT THIS CODE HERE ******
                ImageTextures = []
                if Material.node_tree:
                    ImageTextures.extend([x for x in Material.node_tree.nodes if x.type=='TEX_IMAGE'])
                ...
                #        if Material.texture_slots[TextureSlot].texture.type == 'IMAGE']
                #      
                #    # Refine to only image file names if applicable
                #    ImageFiles = [bpy.path.basename(Texture.image.filepath)
                #        for Texture in ImageTextures
            ...
            # Diffuse = list(Vector(Material.diffuse_color) @ Material.diffuse_intensity) ***** REMOVED IN "2.80" ***** Material.diffuse_intensity default = 0
            Diffuse = list(Vector(Material.diffuse_color))
            #Diffuse.append(Material.alpha)  ***** REMOVED IN "2.80" ***** 

            # Map Blender's range of 1 - 511 to 0 - 1000
            # Specularity = 1000 * (Material.specular_hardness - 1.0) / 510.0 ***** REMOVED IN "2.80" *****
            Specularity = 1000 * (0.0 - 1.0) / 510.0  # Material.specular_hardness defual value = 0
    ...
    def __WriteMeshActiveImageMaterials(self, Mesh):
        ...
        def WriteMaterial(Exporter, MaterialKey):
            ...
            Diffuse = list(Vector(Material.diffuse_color)) #***** REMOVED IN "2.80" ***** Material.diffuse_intensity default = 0
            # Diffuse.append(Material.alpha) #***** REMOVED IN "2.80" ***** Diffuse.append(Material.alpha)
           ...
           #     Exporter.File.Write("TextureFilename {{\"{}\";}}\n".format(bpy.path.basename(Image.filepath)))

            if Image is not None:
                Exporter.File.Write("TextureFilename {{\"{}\";}}\n".format(bpy.path.basename(Image)))
        ...
        def GetMaterialKey(Material, UVTexture, Index):
            ...
            Image = None
            # ************* NEXT TWO not sure if that what it wants ******************
            print("Malc")
            if Material.node_tree:
            ...
            #    Image = UVTexture.data[Index].image if UVTexture.data[Index].image.source == 'FILE' else None
        ...
        #for Index, Polygon in enumerate(Mesh.polygons):
        #    MaterialKey = GetMaterialKey(Materials[Polygon.material_index], Mesh.uv_textures.active, Index) # changed for 2.80

        for Index, Polygon in enumerate(Mesh.polygons):
            MaterialKey = GetMaterialKey(Materials[Polygon.material_index], Mesh.uv_layers.active, Index)
    ...
    def __WriteMeshVertexColors(self, Mesh, MeshEnumerator=None):
        ...
        VertexColors = [VertexColorLayer.data[Index].color for Index in range(0,len(MeshEnumerator.vertices))]
        ...
        for Index, Color in enumerate(VertexColors):
            ...
            self.Exporter.File.Write("{};{:9f};{:9f};{:9f};{:9f};;".format(Index, Color[0], Color[1], Color[2], 1.0))
    ...
    def __WriteMeshSkinWeights(self, Mesh, MeshEnumerator=None):
        ...
        for ArmatureObject in ArmatureObjects:
            ...
            BoneVertexGroups = [_BoneVertexGroup(self.BlenderObject, ArmatureObject, BoneName) for BoneName in UsedBoneNames]
...
class ArmatureExportObject(ExportObject):
    ...
    def __WriteBones(self, Bones):
        ...
        for Bone in Bones:
            ...
            BoneSafeName = self.SafeName + "_" + Util.SafeName(Bone.name)
...
class ArmatureAnimationGenerator(GenericAnimationGenerator):
    ...
    def _GenerateBoneKeys(self):
        ...
        BoneAnimations = [Animation(ArmatureSafeName + "_" + Util.SafeName(Bone.name)) for Bone in ArmatureObject.pose.bones]
        ...
        for Frame in range(Scene.frame_start, Scene.frame_end + 1):
            ...
            for Bone, BoneAnimation in zip(ArmatureObject.pose.bones, BoneAnimations):
                ...
                OldRotation = BoneAnimation.RotationKeys[-1] if len(BoneAnimation.RotationKeys) else Rotation

                Scale = PoseMatrix.to_scale()

                Position = PoseMatrix.to_translation()

                BoneAnimation.RotationKeys.append(Util.CompatibleQuaternion(Rotation, OldRotation))
...
class Util:
    ...
    def WriteMatrix(File, Matrix):
        ...
        File.Write("{:9f},{:9f},{:9f},{:9f},\n".format(Matrix[0][1], Matrix[1][1], Matrix[2][1], Matrix[3][1]))
        File.Write("{:9f},{:9f},{:9f},{:9f},\n".format(Matrix[0][2], Matrix[1][2], Matrix[2][2], Matrix[3][2]))
        File.Write("{:9f},{:9f},{:9f},{:9f};;\n".format(Matrix[0][3], Matrix[1][3], Matrix[2][3], Matrix[3][3]))
